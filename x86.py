...
class cpu:
    ...
    def print_headers(self, procs):
        ...
        if len(self.regtrace) > 0:
            for r in self.regtrace:
                headers.append('%5s' % self.get_regname(r))
    ...
    def print_trace(self, newline):
        trace_output = []  # This will hold everything that needs to be printed
        if len(self.memtrace) > 0:
            for m in self.memtrace:
                if self.compute:
                    if m[0].isdigit():
                        trace_output.append('%5d' % self.memory[int(m)])
                    else:
                        zassert(m in self.vars, 'Traced variable %s not declared' % m)
                        trace_output.append('%5d' % self.memory[self.vars[m]])
                else:
                    trace_output.append('%5s' % '?')
        
        if len(self.regtrace) > 0:
            for r in self.regtrace:
                if self.compute:
                    trace_output.append('%5d' % self.registers[r])
                else:
                    trace_output.append('%5s' % '?')
            ...
            for c in self.condlist:
                if self.compute:
                    if self.conditions[c]:
                        trace_output.append('1 ')
                    else:
                        trace_output.append('0 ')
                else:
                    trace_output.append('? ')
        
        # Join everything into one line with spaces in between
        if trace_output:
            print(" ".join(trace_output), end=" ")
